...
// Helper function to create invoices; invoice_no is generated by DB trigger
export async function createInvoicesWithNumbers(invoicesData: any[]) {
  const { data: invoices, error: insertError } = await supabase
    .from('invoices')
    // invoice_no is intentionally omitted; set by `set_invoice_no_on_insert` trigger
    .insert(invoicesData)
    .select();
...
export const ensureCommunityManagerInvoices = async (userId: string) => {
  try {
    ...
    const communityMeta = managerCommunities.map(row => {
