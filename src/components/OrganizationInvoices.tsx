...
export function OrganizationInvoices() {
  ...
  const { invoices, loading: invoicesLoading } = useOrganizationInvoices()
  ...
  const handleDownload = (invoiceId: string) => {
    ...
    // Use user profile name or a default "Organization Manager"
    const billToName = `${user.profile?.first_name || ''} ${user.profile?.last_name || ''}`.trim() || 'Organization Manager'
  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        <div className="p-6 bg-white rounded-lg shadow-sm text-gray-600">No invoices found for your organization.</div>
      ) : (
