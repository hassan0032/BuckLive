    ...
    <div>
      <div className="mb-6">
        ...
        <div className="text-sm text-gray-600 mt-2">
          <div>Billing start: <span className="font-medium">{startDate ? new Date(startDate).toLocaleDateString() : '-'}</span></div>
          <div>Next renewal: <span className="font-medium">{renewalDate ? new Date(renewalDate).toLocaleDateString() : '-'}</span></div>
        </div>
      </div>
      {rows.length === 0 ? (
          ...
          {rows.map((row) => (
            <div key={row.id} className="grid grid-cols-6 gap-4 px-4 py-4 items-center border-b last:border-b-0">
              <div className="col-span-2">
                <div className="text-[#363f49]">{new Date(row.periodStart).toLocaleDateString()} – {new Date(row.periodEnd).toLocaleDateString()}</div>
                <div className="text-xs text-gray-500">ID: {row.id.slice(0, 8)}…</div>
              </div>
              <div className="text-gray-700 flex items-center gap-2"><Calendar className="w-4 h-4" /> {new Date(row.issueDate).toLocaleDateString()}</div>
